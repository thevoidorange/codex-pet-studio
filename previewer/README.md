...
**Keyframes & Timing** combines frame inspection and duration editing in the right rail. Select a keyframe to inspect it and edit its duration in 5 ms steps, with a deliberate per-frame guardrail of 20–5000 ms. Runtime Simulation, loop totals, and the Production Timing Board update immediately. The selected timing frame stays pinned while playback continues.

The editor deliberately does not write the fetched config, retain changes in local storage, mutate a private `build/` file, regenerate a GIF, or alter the installed Codex pet package. **Copy Changes For Codex** puts a minimal override payload on the clipboard for pasting into a Codex task. **Download Timing JSON** saves the same payload as `timing-overrides.json`. Both contain only changed state IDs and duration arrays. These timings are shared across every visual version because `states[].durations` is project-level metadata.

The 5 ms value is a fine adjustment for Preview review, not a promise that browser rendering or the Codex client can deliver every transition at that exact instant. GIF frame delay uses 10 ms units, so values ending in 5 ms must be converted to a 10 ms value during a fresh GIF export.

The bundled example uses `idleSlowdown: 6` to make Runtime Simulation return to a visibly calmer Idle after an action. That multiplier is a Previewer choice, not a Codex pet-pack requirement or technical limit; external preview configs can choose another value.
