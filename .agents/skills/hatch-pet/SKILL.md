---
name: hatch-pet
description: Compile, repair, validate, visually QA, and package Codex-compatible v2 animated pets after Pet Studio has approved the identity, mechanisms, state acting, and motion handoff, or when repairing an existing pack. Do not use for cold-start inspiration or early concept exploration. Use for the 8x11 production workflow requiring all 9 standard animation rows, 16 look directions, deterministic assembly, QA artifacts, and spriteVersionNumber 2 packaging.
---

# Hatch Pet

> Modified for Codex Pet Studio on 2026-07-31: this vendored snapshot can run
> from its project-local skill directory, acts as a subordinate production
> compiler rather than a cold-start concept workflow, defaults private run
> artifacts to `build/`, and is governed by the Studio's build-first,
> explicit-install boundary. The upstream Apache License 2.0 is preserved in
> `LICENSE.txt`.

## Overview

This project-bundled variant is the production compiler for an approved Pet
Studio handoff. For a new inspiration or concept request, route through
`$pet-studio` first and do not start a full atlas from this skill. Once the
identity, mechanisms, state acting, and motion language are approved, compile
the supplied character and grounding assets into a Codex-compatible v2 pet.
Every newly hatched pet is an 8x11 atlas with the 9 standard animation rows
plus 16 clockwise look directions and is packaged with
`spriteVersionNumber: 2`. The intermediate 8x9 atlas exists only to assemble
and review rows 0-8; never package it as a new pet.
...
Look rows must have transparent backgrounds after assembly. Do not accept or package the pet if `qa/look-directions.png` or `qa/contact-sheet-extended.png` shows chroma-key panels behind any look cell. If generated look rows contain slight chroma-key lighting variation, rerun assembly with a wider `--chroma-threshold` instead of packaging the opaque key color. Validation must pass without opaque chroma-key-pixel errors.
...
4. Major failures require repair. Minor failures may be overridden and the packaging pipeline continues.
...
Only after all deterministic and visual QA passes, stage the approved extended
spritesheet as a v2 pet. `spriteVersionNumber: 2` is mandatory; without it the
app defaults to the 9-row v1 contract and rejects the 2288-pixel-tall asset.

When this skill runs inside Codex Pet Studio, set `PET_OUTPUT_DIR` to the
project's absolute `build/pet` path. Otherwise the safe default stays inside
the private run directory. Packaging never writes directly to a live Codex
pets directory. Installation is a separate, explicit operation owned by
`studio.py install`.
...
- the selected package directory contains `pet.json` and `spritesheet.webp`
  together; in Codex Pet Studio this is the private `build/pet` directory.
